...
import { gradeParaphrase } from '@/lib/passages/ai/grade-paraphrase';
  ...
  try {
    ...
    if (input.length < MIN_INPUT_LEN) {
      return NextResponse.json(
        { error: `Bài viết quá ngắn (≥ ${MIN_INPUT_LEN} ký tự).` },
        { status: 400 },
      );
    }
    ...
    const feedback = await gradeParaphrase(passage.content, input);
    if (!feedback) {
    ...
    console.error('[passage paraphrase-grade] error:', err);
